...
from Joystick_virtual import Joystick_virtual
import os 
...
class Zao_sdk_App(customtkinter.CTk):
    def __init__(self):
        ...
        right_middle=customtkinter.CTkFrame(self,width=400,height=250,border_width=1)
        ...
        label_port=customtkinter.CTkLabel(Left,text='Port:')
        label_port.grid(row=4, column=0, padx=10, pady=2, sticky="w" )
        self.label_port_value=customtkinter.CTkLabel(Left,text='Not selected')
        self.label_port_value.grid(row=4, column=0, padx=15, pady=2)

      
        label_Baud=customtkinter.CTkLabel(Left,text='Baud:')
        label_Baud.grid(row=5, column=0, padx=10, pady=2, sticky="w" )
        label_Baud_value=customtkinter.CTkLabel(Left,text=BAUD)
        label_Baud_value.grid(row=5, column=0, padx=15, pady=2 )

        label_Freq=customtkinter.CTkLabel(Left,text='Freq.:')
        label_Freq.grid(row=6, column=0, padx=10, pady=2, sticky="w" )
        label_Freq_value=customtkinter.CTkLabel(Left,text=FREQUENCY)
        label_Freq_value.grid(row=6, column=0, padx=15, pady=2, )
        ...
        self.lable_throttle.pack(side='left')
    ...
    #   start connection to port 
    #   stop the coonection = False 
            self.start_new_thread('False',self.selected_port)
            # self.listbox.grid_remove()
            
        elif not Selected:
            # print("please select the port")
            self.label_port_value.configure(text="Not Selected",fg_color="red")
            # self.label_port['text']=f'Not Connect'
            # showinfo(title="Port not selected", message='Please select one of avaible serial port',)
    ...
    #   Stop connection to port =Ture
            # self.start_new_thread('True',selected_port)
    ...
    def update_lables(self):
        ...
        if self.zaosdkcontroller.get_joy_details()[1]==1:
            ...
            # print(f"sdfsdfdsf{self.zaosdkcontroller.get_direction()}")
        else:
            self.label_Joy_value.configure(text="Disconnected", fg_color="red") 
        self.after(1000, self.update_lables) # run itself again after 1000 ms

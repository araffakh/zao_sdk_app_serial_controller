...
class Zao_sdk_App(customtkinter.CTk):
    def __init__(self):
        ...
        Left=customtkinter.CTkFrame(self,width=200,height=400,fg_color="white")
        Left.grid(row=0, column=0, padx=10, pady=10, sticky="nw",rowspan=8,)
                
        right_top=customtkinter.CTkFrame(self,width=400,height=50,border_width=1,fg_color="#2B2B2B")
        right_top.grid(row=0, column=1, padx=10, pady=10, sticky="nsew")
        
        right_middle=customtkinter.CTkFrame(self,width=400,height=100,border_width=1,fg_color="#2B2B2B")
        right_middle.grid(row=1, column=1, padx=10, pady=10,sticky="nsew")
        
        right_bottom=customtkinter.CTkFrame(self,width=400,height=50,border_width=1,fg_color="#2B2B2B")
        right_bottom.grid(row=2, column=1, padx=10, pady=10,sticky="nsew")
      
        my_image = customtkinter.CTkImage(light_image=Image.open(self.resource_path0("logo.png")),
                                  dark_image=Image.open(self.resource_path0("logo.png")),
                                  size=(50, 50))
        my_image_txt = customtkinter.CTkImage(light_image=Image.open(self.resource_path0("logo_text_small.png")),
                                  dark_image=Image.open(self.resource_path0("logo_text_small.png")),
                                  size=(100, 11))
        ...
        label_logo.grid(row=0, column=0, padx=0, pady=(10,0), sticky="n" )
        
        label_logo_txt=customtkinter.CTkLabel(Left, image=my_image_txt, text='')
        label_logo_txt.grid(row=1, column=0, padx=0, pady=2, sticky="n" )
        
        self.listbox=customtkinter.CTkOptionMenu(Left,width=150,height=30,variable=self.var, values=list_avaialble_ports )
        self.listbox.grid(row=2, column=0, padx=15, pady=(30,0), sticky="n" )
        
        self.BT_connect=customtkinter.CTkButton(Left,text="connect",command=  lambda: self.items_selected(True))
        self.BT_connect.grid(row=3, column=0, padx=0, pady=2, sticky="n" )

        self.label_connection=customtkinter.CTkLabel(Left,text='Not Connected',bg_color="red")
        self.label_connection.grid(row=4, column=0, padx=0, pady=(30,10), sticky="n")
        
        self.font_bold=customtkinter.CTkFont("",13,"bold")
        self.font_general=customtkinter.CTkFont("",13)

        label_port=customtkinter.CTkLabel(right_middle,text='Port:',font=self.font_bold)
        label_port.grid(row=0, column=0, padx=(10,5), pady=0, sticky="NW" )
        
        self.label_port_value=customtkinter.CTkLabel(right_middle,text='Not selected')
        self.label_port_value.grid(row=0, column=1, padx=(10,2), pady=0,sticky="NW")

      
        label_Baud=customtkinter.CTkLabel(right_middle,text='Baud:',font=self.font_bold)
        label_Baud.grid(row=0, column=2, padx=(10,2), pady=0, sticky="N" )
        label_Baud_value=customtkinter.CTkLabel(right_middle,text=BAUD)
        label_Baud_value.grid(row=0, column=3, padx=(10,2), pady=0,sticky="N" )

        label_Freq=customtkinter.CTkLabel(right_middle,text='Freq.: ',font=self.font_bold)
        label_Freq.grid(row=0, column=4, padx=(10,5), pady=0, sticky="NE" )
        label_Freq_value=customtkinter.CTkLabel(right_middle,text=FREQUENCY)
        label_Freq_value.grid(row=0, column=5, padx=(10,2), pady=0,sticky="NE")

      
        
        # self.label_tx=customtkinter.CTkLabel(right_middle,text='TX')
        # self.label_tx.grid(row=2, column=1, padx=(10,5), pady=2, sticky="w" )
        self.label_tx_value=customtkinter.CTkLabel(right_middle,text="none")
        self.label_tx_value.grid(row=1, column=2, padx=(10,5), pady=2 ,sticky="nsew")
        
        # self.label_rx=customtkinter.CTkLabel(right_middle,text='RX')
        # self.label_rx.grid(row=2, column=3, padx=(10,5), pady=2, sticky="w" )
        self.label_rx_value=customtkinter.CTkLabel(right_middle,text="none")
        self.label_rx_value.grid(row=1, column=3, padx=(10,5), pady=2 ,sticky="nsew")
        ...
        label_Joy.grid(row=0, column=1, padx=10, pady=0, sticky='w' )
        self.label_Joy_value=customtkinter.CTkLabel(right_top,text='connected',fg_color='green')
        self.label_Joy_value.grid(row=0, column=2, padx=15, pady=0,sticky='w'  )
        ...
        label_app_info=customtkinter.CTkLabel(right_bottom,text='Zao SDK Controller App for JetRacer ROS(ProximaVision) - V1.1.0.0\n Khaldon Araffa 2023',font=("",9))
        label_app_info.grid(row=0, column=0, padx=80, pady=0,sticky="N")
        
        self.BT_connect_exit=customtkinter.CTkButton(right_bottom,text="Exit",command=lambda:self.exit_app())
        self.BT_connect_exit.grid(row=1, column=0, padx=0, pady=0, sticky="S" )
        ...
        # self.lable_throttle.grid(row=4, column=1, padx=0, pady=5,sticky="N")
    ...
    def exit_app(self):
        self.start_new_thread('TRUE',self.selected_port)
        self.quit()        
    ...
    def update_lables(self):
        ...
        if self.zaosdkcontroller.get_joy_details()[1]==1:
            ...
            if self.zaosdkcontroller.get_leds()[0]==True:
                ...
                self.label_tx_value.configure(text="TX",fg_color="red")
            ...
            if self.zaosdkcontroller.get_leds()[1]==True:
                ...
                self.label_rx_value.configure(text="RX",fg_color="red")
